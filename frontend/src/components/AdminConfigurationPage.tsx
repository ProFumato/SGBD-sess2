...
export function AdminConfigurationPage() {
  ...
  return <section className="content-card" aria-labelledby="config-title">
    ...
    {global && <form onSubmit={(event) => { event.preventDefault(); if (siteName.trim()) void action(() => createSite(actor, siteName.trim())); setSiteName(""); }}>
      <h3>Create site</h3><label htmlFor="site-name">Name</label><input id="site-name" value={siteName} onChange={(event) => setSiteName(event.target.value)} /><button className="button" disabled={busy}>Create site</button>
    </form>}
    <label htmlFor="config-site">Selected site</label><select id="config-site" value={siteId ?? ""} onChange={(event) => setSiteId(Number(event.target.value))}>{sites.map((site) => <option key={site.siteId} value={site.siteId}>{site.name} (#{site.siteId})</option>)}</select>
    {siteId && <><form onSubmit={(event) => { event.preventDefault(); const site = sites.find((item) => item.siteId === siteId); if (site) void action(() => updateSite(actor, siteId, siteName.trim() || site.name)); }}>
      <h3>Site details</h3><label htmlFor="selected-site-name">Site name</label><input id="selected-site-name" value={siteName || sites.find((site) => site.siteId === siteId)?.name || ""} onChange={(event) => setSiteName(event.target.value)} /><button className="button" disabled={busy}>Save site</button>
