...
export function PublicMatchesPage() {
  ...
  async function loadMatches() {
    ...
    setLoading(true);
    try {
      setMatches(await getPublicMatches(identity.member.matricule));
      setError(null);
    } catch (caughtError) {
  ...
  return (
    <section className="content-card" aria-labelledby="public-matches-title">
      ...
      <div className="filter-row">
        <label htmlFor="public-site-filter">Site ID</label>
        <input id="public-site-filter" type="number" min="1" value={siteFilter} onChange={(event) => setSiteFilter(event.target.value)} />
        ...
        {filteredMatches.map((match) => (
          <article className="public-match-card" key={match.matchId}>
            <div>
              ...
              <p>
                Site {match.siteId} · {match.courtName} · {formatBrusselsDateTime(match.startsAt)}–{formatBrusselsTime(match.endsAt)}
              </p>
